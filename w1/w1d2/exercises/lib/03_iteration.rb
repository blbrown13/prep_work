...
class Array
  def bubble_sort!(&prc)
    ...
    self.map.with_index do |ele1, idx1|

      self.map.with_index do |ele2, idx2|
...
subs = []
  ...
  letters.each.with_index do |letter,index|
    ...
    while i < letters.length do
      s = letters[index..i].join('')
      subs << s unless subs.include?(s)
      i += 1
    end
  end
