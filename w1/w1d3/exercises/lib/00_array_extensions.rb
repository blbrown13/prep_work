...
class Array
  def median
    ...
    if array.length == 1
      ...
      (array[(array.length/2).floor] + array[(array.length/2)-1])/2.0
    end
...
class Array
  def my_transpose
    ...
    0.upto(self.length-1) do |i|
      ...
      0.upto(row.length-1) do |j|
        foo << self[j][i]
      end
    end

    until foo.empty?
      trans << foo.slice!(0,self.length)
    end
